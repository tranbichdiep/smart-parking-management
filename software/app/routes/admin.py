...
def user_management():
    ...
    if q:
        like_term = f"%{escape_like(q)}%"
        conditions.append("(username LIKE ? ESCAPE '\\' OR full_name LIKE ? ESCAPE '\\' OR employee_code LIKE ? ESCAPE '\\')")
        params.extend([like_term, like_term, like_term])
...
@admin_bp.route("/admin/users/delete/<username>")
@login_required
@role_required("admin")
...
@admin_bp.route("/admin/users/toggle_status/<username>")
@login_required
@role_required("admin")
...
@admin_bp.route("/admin/add_card", methods=["POST"])
@login_required
@role_required("admin")
def add_card():
    ...
    try:
        conn.execute(
            ...
            (
                card_id,
                holder_name if ticket_type == "monthly" else "N/A",
                license_plate if ticket_type == "monthly" else None,
                ticket_type,
                expiry_date,
                created_at,
                "active",
...
@admin_bp.route("/admin/edit_card", methods=["POST"])
@login_required
@role_required("admin")
def edit_card():
    ...
    try:
        ...
        if ticket_type == "monthly" and extend_months > 0:
            try:
                base_date = (
                    datetime.strptime(base_date_str, "%Y-%m-%d %H:%M:%S") if base_date_str else datetime.now()
                )
            except ValueError:
        ...
        conn.rollback()
        flash(f"Lỗi khi cập nhật thẻ: {exc}", "danger")
    finally:
        conn.close()
    return redirect(url_for("admin.admin_dashboard"))


@admin_bp.route("/admin/cards/set_status", methods=["POST"])
@login_required
@role_required("admin")
def set_card_status():
    ...
    try:
        ...
        conn.rollback()
        flash(f"Lỗi khi cập nhật trạng thái thẻ: {exc}", "danger")
    finally:
        conn.close()
...
@admin_bp.route("/admin/delete_card/<card_id>")
@login_required
@role_required("admin")
...
def statistics():
    ...
    revenue_today = (
        conn.execute("SELECT SUM(fee) FROM transactions WHERE date(exit_time) = ? AND fee IS NOT NULL", (today_str,)).fetchone()[0]
        or 0
    )

    traffic_today = (
        conn.execute("SELECT COUNT(*) FROM transactions WHERE date(entry_time) = ?", (today_str,)).fetchone()[0] or 0
    )

    cars_in_parking = conn.execute("SELECT COUNT(*) FROM transactions WHERE exit_time IS NULL").fetchone()[0] or 0
